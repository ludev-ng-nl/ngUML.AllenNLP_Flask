...
class Coreference:
   ...
   def replace_partial_text_with_coref(self,avo_sen:list,result_per_avo_sent:dict,
                                       avo_results_index:int,input_text:list) -> list:
   ...
   def order_coref_on_avo_index(self,avo_sents:list,coref_results:list,coref_text:list) -> dict:
      ...
      for reference_result in coref_results:
         ...
         found_avo_indices = [index for index, test_range in enumerate(possible_ranges)
         ...
         for avo_sent_index in avo_sent_indices:
            ...
            reference_result_item[2] = [reference_result_item[2],begin_reference_sent_index,
   ...
   def fill_swimming_lanes_and_coref_sents(self,avo_sents:list,coref_results:list,
                                             coref_text:list) -> list:
      ...
      for avo_index, avo_sent in enumerate(avo_sents):
         ...
         if avo_sent['sw_lane']:
            ...
            swimming_lane_in_sent = [item if index in range(begin_sw,end_sw) else "###"
                                       for index,item in enumerate(action_text)]
            coref_swimming_lane_result = self.replace_partial_text_with_coref(avo_sent,
                                                result_per_avo_sent,avo_index,swimming_lane_in_sent)
            swimming_lane = [word for word in coref_swimming_lane_result if word != "###"]
            #complete sent
            coref_complete_sent = self.replace_partial_text_with_coref(avo_sent,result_per_avo_sent,
